...
def viualization_featureMap():
    args = parse_args()
    model = archs.__dict__[args.arch](args, 10)
    ...
    for i, (input, target) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        print('original image:', img.detach().cpu().numpy().shape)
        print('predict image:', pred.detach().cpu().numpy().shape)
...
if __name__ == '__main__':
    ...
    # 修改archs.py中的def forward()，return self.features(input)
    # viualization_featureMap()

...
class ScaledMNIST(Dataset):
    def __init__(self, train=True, transform=None, target_transform=None):
        ...
        if not os.path.exists('input/scaled_cifar10_train.npz'):
            ...
            np.savez('input/scaled_cifar10_train.npz', images=scaled_train_imgs, labels=train_labels)

        if not os.path.exists('input/scaled_cifar10_test.npz'):
            ...
            np.savez('input/scaled_cifar10_test.npz', images=scaled_test_imgs, labels=test_labels)

        if self.train:
            scaled_mnist_train = np.load('input/scaled_cifar10_train.npz')
            self.train_data = scaled_mnist_train['images']
            self.train_labels = scaled_mnist_train['labels']
        else:
            scaled_mnist_test = np.load('input/scaled_cifar10_test.npz')
